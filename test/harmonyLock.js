...
const HarmonyLock = artifacts.require('HarmonyLock.sol');

let token, harmonyLock

contract('HarmonyLock', async (accounts) => {
    // Prepare Accounts
    ...
    const secretAutoA1 = "0x9f7259d05ac4a50c60d298bb1bf3c3821f0c88fff298ffdf37bc7304400535d7"
    const secretHashAutoA1 = "0x55de4870a654f460a6deafe89d5a1e6dba380a8cae222a90ff3fee5902d1eaad"
    ...
    const wrongSecretHash = 'WRONG_SECRET_HASH'
    ...
    beforeEach(async () => {
        ...
        harmonyLock = await HarmonyLock.new()

        // Create a New Loan Request
        await harmonyLock.lockCollateral(
            lender,
            secretHashA1,
            secretHashB1,
            secretHashAutoA1,
    ...
    describe('Fetch Loan', () => {
        it('should fetch loan details', async () => {
            const loanInfo = await harmonyLock.fetchLoan(1)
            // console.log(loanInfo)
            assert.equal(lender, loanInfo.actors[1])
        })
    })

    describe('Unlock & Close', async () => {
        it('should unlock collateral and close loan', async () => {
            await harmonyLock.unlockCollateralAndCloseLoan(1, secretB1)
            const loanInfo = await harmonyLock.fetchLoan(1)
            assert.equal(loanInfo.state.toString(), '1')
        })
    })

    describe('Unlock Refundable Collateral', async () => {
        it('should unlock refundable collateral', async () => {
            currentTime = parseInt(await time.latest())
            ...
            // Unlock Refundable Collateral
            await harmonyLock.unlockRefundableCollateral(2)

            const loanInfo = await harmonyLock.fetchLoan(2)
            // console.log(loanInfo)
            assert.equal(parseInt(loanInfo.details[1]), 0)
        })
    })

    describe('Unlock Seizable Collateral', async () => {
        it('should unlock seizable collateral', async () => {
            currentTime = parseInt(await time.latest())

            loanExpiration = currentTime - loanIncrement
            seizureExpiration = currentTime - seizureIncrement
            ...
            // Unlock Refundable Collateral
            await harmonyLock.unlockSeizableCollateral(2, secretA1)

            const loanInfo = await harmonyLock.fetchLoan(2)
            // console.log(loanInfo)
            assert.equal(parseInt(loanInfo.details[0]), 0)
        })
    })

    describe('Seize Collateral', async () => {
        it('should seiza seizable collateral', async () => {
            currentTime = parseInt(await time.latest())
            ...
            // Unlock Refundable Collateral
            await harmonyLock.seizeCollateral(2, secretA1)
